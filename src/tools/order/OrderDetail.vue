<template>
  <div class="OrderDetail bodybox">
    ...
    <div style="margin-bottom:40px;">
      <van-cell is-link :value="orderData.kyc_form==0?'Not Filled':'Filled'" title-style="text-align:left;" title="KYC" @click="$store.commit('changePage',{tabbar: '/KYC', title: 'KYC'});$router.push({path: '/KYC',query: {orderId: orderData.id}})"></van-cell>
      <van-cell is-link :value="orderData.letter_of_wishes_form==0?'Not Filled':'Filled'" title-style="text-align:left;" title="Letter Of Wishes" @click="$store.commit('changePage',{tabbar: '/LetterOfWishes', title: 'Letter Of Wishes'});$router.push({path: '/LetterOfWishes', query: {orderId: orderData.id}})"></van-cell>
      <van-cell is-link :value="orderData.pdpa_memo_form==0?'Not Filled':'Filled'" title-style="text-align:left;" title="PDPA Memo" @click="$store.commit('changePage',{tabbar: '/PDPAMemo', title: 'PDPA Memo'});$router.push({path: '/PDPAMemo', query: {orderId: orderData.id}})"></van-cell>
      <van-cell is-link :value="orderData.third_party_declaration_form==0?'Not Filled':'Filled'" title-style="text-align:left;" title="Third Party Declaration (If Applicable)" @click="$store.commit('changePage',{tabbar: '/ThirdPartyDeclaration', title: 'Third Party Declaration'});$router.push({path: '/ThirdPartyDeclaration',query: {orderId: orderData.id} })"></van-cell>
    </div>
    ...
    />
    <van-button class="loginBtn" type="info" @click="submit">Submit Feedback</van-button>
  </div>
</template>
