<template>
  <div class="DocumentChecklist bodybox">
    ...
    <van-form validate-first @submit="submit" :submit-on-enter="false" :scroll-to-error="true">
      ...
      <van-field name="name" label="Name" center autocomplete="off" oninput="
  var start = this.selectionStart;
  var end = this.selectionEnd;
  this.value = this.value.toUpperCase();
  this.setSelectionRange(start, end);" :required="true" v-model="formData.name"
        :rules="[{ required: true, message: 'Please enter the name' }]" />
      <van-field name="nric" label="NRIC/Passport No" center oninput="
  var start = this.selectionStart;
  var end = this.selectionEnd;
  this.value = this.value.toUpperCase();
  this.setSelectionRange(start, end);" :required="true" autocomplete="off" v-model="formData.nric"
        :rules="[{ required: true, message: 'Please enter the NRIC/Passport No' }]" />
      <div class="minTitle">Documents Completion Status</div>
      <van-field name="application_form" label="Completed Cash Trust application form" center>
        <template #input>
          <van-checkbox v-model="formData.application_form"></van-checkbox>
        </template>
      </van-field>
      <van-field name="compliance_qestionnaire"
        label="Completed and signed Compliance Questionnaire as provided by AITB: a) AITB-AMLA 2(A)" center>
        <template #input>
          <van-checkbox v-model="formData.compliance_qestionnaire"></van-checkbox>
        </template>
      </van-field>
      <van-field name="settlor_photocopy" label="Settlor’s Photocopy of IC (Verified by the Agent/Representative)" center>
        <template #input>
      ...
      <van-field name="Documents" label="Documents">
        <template #input>
          ...
          <van-uploader v-model="settlor_photo_file" :disabled="isDone" :before-delete="beforeRead"
            :after-read="afterRead" accept="*" :max-count="1" @click-preview="goDetail1(settlor_photo_file)" />
        </template>
      </van-field>
      <van-field name="letter_of_wishes" label="Signed Letter of Wishes (Un-dated)" center>
        <template #input>
          <van-checkbox v-model="formData.letter_of_wishes"></van-checkbox>
        </template>
      </van-field>
      <van-field name="current_address"
      ...
      <van-field name="Documents" label="Documents">
        <template #input>
          <van-uploader v-model="proof_of_current_file" :after-read="afterRead2" :before-delete="beforeRead2" accept="*"
            :max-count="1" :disabled="isDone" @click-preview="goDetail1(proof_of_current_file)" />
        </template>
      </van-field>
      <van-field name="source_of_wealth"
      ...
      <van-field name="Documents" label="Documents">
        ...
        <template #input v-if="this.isFilled == 0 || this.isFilled == undefined">
          ...
          <van-uploader v-model="source_of_wealth_file1w" :after-read="afterRead31w" :before-delete="beforeRead31w"
            accept="*" :max-count="1" :disabled="isDone" />
          <van-uploader v-model="source_of_wealth_file2w" :after-read="afterRead32w" :before-delete="beforeRead32w"
            accept="*" :max-count="1" :disabled="isDone" />
          <van-uploader v-model="source_of_wealth_file3w" :after-read="afterRead33w" :before-delete="beforeRead33w"
            accept="*" :max-count="1" :disabled="isDone" />
          <van-uploader v-model="source_of_wealth_file4w" :after-read="afterRead34w" :before-delete="beforeRead34w"
            accept="*" :max-count="1" :disabled="isDone" />
          <van-uploader v-model="source_of_wealth_file5w" :after-read="afterRead35w" :before-delete="beforeRead35w"
            accept="*" :max-count="1" :disabled="isDone" />

        </template>
        <template #input v-else>

          <van-uploader v-model="source_of_fund_addon1w" @click-preview="goDetail1(source_of_fund_addon1w)"
            :before-delete="beforeRead81w" :after-read="afterRead81w" accept="*" :max-count="1" :disabled="isDone" />
          <van-uploader v-model="source_of_fund_addon2w" @click-preview="goDetail1(source_of_fund_addon2w)"
            :before-delete="beforeRead91w" :after-read="afterRead91w" accept="*" :max-count="1" :disabled="isDone" />
          <van-uploader v-model="source_of_fund_addon3w" @click-preview="goDetail1(source_of_fund_addon3w)"
            :before-delete="beforeRead101w" :after-read="afterRead101w" accept="*" :max-count="1" :disabled="isDone" />
          <van-uploader v-model="source_of_fund_addon4w" @click-preview="goDetail1(source_of_fund_addon4w)"
            :before-delete="beforeRead111w" :after-read="afterRead111w" accept="*" :max-count="1" :disabled="isDone" />
          <van-uploader v-model="source_of_fund_addon5w" @click-preview="goDetail1(source_of_fund_addon5w)"
            :before-delete="beforeRead112w" :after-read="afterRead112w" accept="*" :max-count="1" :disabled="isDone" />
        </template>


        <!-- </template> -->
      </van-field>
      <van-field name="source_of_fund" label="Source of Fund (to provide the relevant supporting document(s)) 
Verified by the Agent/ Representative – Bank Statement, Bank passbook" center>
      ...
      <van-field name="Documents" label="Documents">
        <template #input v-if="this.isFilled == 0 || this.isFilled == undefined">
          ...
          <van-uploader v-model="source_of_wealth_file1" :after-read="afterRead31" :before-delete="beforeRead31"
            accept="*" :max-count="1" :disabled="isDone" />
          <van-uploader v-model="source_of_wealth_file2" :after-read="afterRead32" :before-delete="beforeRead32"
            accept="*" :max-count="1" :disabled="isDone" />
          <van-uploader v-model="source_of_wealth_file3" :after-read="afterRead33" :before-delete="beforeRead33"
            accept="*" :max-count="1" :disabled="isDone" />
          <van-uploader v-model="source_of_wealth_file4" :after-read="afterRead34" :before-delete="beforeRead34"
            accept="*" :max-count="1" :disabled="isDone" />
          <van-uploader v-model="source_of_wealth_file5" :after-read="afterRead35" :before-delete="beforeRead35"
            accept="*" :max-count="1" :disabled="isDone" />

        </template>
        <template #input v-else>

          <van-uploader v-model="source_of_fund_addon1" @click-preview="goDetail1(source_of_fund_addon1)"
            :before-delete="beforeRead81" :after-read="afterRead81" accept="*" :max-count="1" :disabled="isDone" />
          <van-uploader v-model="source_of_fund_addon2" @click-preview="goDetail1(source_of_fund_addon2)"
            :before-delete="beforeRead91" :after-read="afterRead91" accept="*" :max-count="1" :disabled="isDone" />
          <van-uploader v-model="source_of_fund_addon3" @click-preview="goDetail1(source_of_fund_addon3)"
            :before-delete="beforeRead101" :after-read="afterRead101" accept="*" :max-count="1" :disabled="isDone" />
          <van-uploader v-model="source_of_fund_addon4" @click-preview="goDetail1(source_of_fund_addon4)"
            :before-delete="beforeRead111" :after-read="afterRead111" accept="*" :max-count="1" :disabled="isDone" />
          <van-uploader v-model="source_of_fund_addon5" @click-preview="goDetail1(source_of_fund_addon5)"
            :before-delete="beforeRead112" :after-read="afterRead112" accept="*" :max-count="1" :disabled="isDone" />
        </template>
      </van-field>
      <van-field name="pdpa" label="Signed dual language personal data protection act (PDPA) document" center>
        <template #input>
      ...
      <van-field name="Documents" label="Documents">
        <template #input v-if="this.isFilled == 0 || this.isFilled == undefined">
          ...
          <van-uploader v-model="source_of_wealth_file1y" :after-read="afterRead31y" :before-delete="beforeRead31y"
            accept="*" :max-count="1" :disabled="isDone" />
          <van-uploader v-model="source_of_wealth_file2y" :after-read="afterRead32y" :before-delete="beforeRead32y"
            accept="*" :max-count="1" :disabled="isDone" />
          <van-uploader v-model="source_of_wealth_file3y" :after-read="afterRead33y" :before-delete="beforeRead33y"
            accept="*" :max-count="1" :disabled="isDone" />
          <van-uploader v-model="source_of_wealth_file4y" :after-read="afterRead34y" :before-delete="beforeRead34y"
            accept="*" :max-count="1" :disabled="isDone" />
          <van-uploader v-model="source_of_wealth_file5y" :after-read="afterRead35y" :before-delete="beforeRead35y"
            accept="*" :max-count="1" :disabled="isDone" />

        </template>
        <template #input v-else>

          <van-uploader v-model="source_of_fund_addon1y" @click-preview="goDetail1(source_of_fund_addon1y)"
            :before-delete="beforeRead81y" :after-read="afterRead81y" accept="*" :max-count="1" :disabled="isDone" />
          <van-uploader v-model="source_of_fund_addon2y" @click-preview="goDetail1(source_of_fund_addon2y)"
            :before-delete="beforeRead91y" :after-read="afterRead91y" accept="*" :max-count="1" :disabled="isDone" />
          <van-uploader v-model="source_of_fund_addon3y" @click-preview="goDetail1(source_of_fund_addon3y)"
            :before-delete="beforeRead101y" :after-read="afterRead101y" accept="*" :max-count="1" :disabled="isDone" />
          <van-uploader v-model="source_of_fund_addon4y" @click-preview="goDetail1(source_of_fund_addon4y)"
            :before-delete="beforeRead111y" :after-read="afterRead111y" accept="*" :max-count="1" :disabled="isDone" />
          <van-uploader v-model="source_of_fund_addon5y" @click-preview="goDetail1(source_of_fund_addon5y)"
            :before-delete="beforeRead112y" :after-read="afterRead112y" accept="*" :max-count="1" :disabled="isDone" />
        </template>
      </van-field>


      <van-field name="beneficiary_photocopy" label="Beneficiary's Photocopy of IC (Verified by the Agent/Representative)"
        center>
        <template #input>
      ...
      <van-field name="Documents" label="Documents">
        <template #input v-if="this.isFilled == 0 || this.isFilled == undefined">
          ...
          <van-uploader v-model="source_of_wealth_file1x" :after-read="afterRead31x" :before-delete="beforeRead31x"
            accept="*" :max-count="1" :disabled="isDone" />
          <van-uploader v-model="source_of_wealth_file2x" :after-read="afterRead32x" :before-delete="beforeRead32x"
            accept="*" :max-count="1" :disabled="isDone" />
          <van-uploader v-model="source_of_wealth_file3x" :after-read="afterRead33x" :before-delete="beforeRead33x"
            accept="*" :max-count="1" :disabled="isDone" />
          <van-uploader v-model="source_of_wealth_file4x" :after-read="afterRead34x" :before-delete="beforeRead34x"
            accept="*" :max-count="1" :disabled="isDone" />
          <van-uploader v-model="source_of_wealth_file5x" :after-read="afterRead35x" :before-delete="beforeRead35x"
            accept="*" :max-count="1" :disabled="isDone" />
        ...
        <template #input v-else>
          ...
          <van-uploader v-model="beneficiary_photo_file1" @click-preview="goDetail1(beneficiary_photo_file1)"
            :before-delete="beforeRead7" :after-read="afterRead7" accept="*" :max-count="1" :disabled="isDone" />
          <van-uploader v-model="beneficiary_photo_file2" @click-preview="goDetail1(beneficiary_photo_file2)"
            :before-delete="beforeRead8" :after-read="afterRead8" accept="*" :max-count="1" />
          <van-uploader v-model="beneficiary_photo_file3" @click-preview="goDetail1(beneficiary_photo_file3)"
            :before-delete="beforeRead9" :after-read="afterRead9" accept="*" :max-count="1" :disabled="isDone" />
          <van-uploader v-model="beneficiary_photo_file4" @click-preview="goDetail1(beneficiary_photo_file4)"
            :before-delete="beforeRead10" :after-read="afterRead10" accept="*" :max-count="1" :disabled="isDone" />
          <van-uploader v-model="beneficiary_photo_file5" @click-preview="goDetail1(beneficiary_photo_file5)"
            :before-delete="beforeRead11" :after-read="afterRead11" accept="*" :max-count="1" :disabled="isDone" />
        </template>
      </van-field>
      <!-- <van-button
          class="esignDelBtn"
      ...
      <van-field name="third_party_declaration" label="Third Party Fund Declaration Form (if applicable)" center
      ...
      <van-field v-model="formData.agent_name" id="5reset" name="agent_name" oninput="
  var start = this.selectionStart;
  var end = this.selectionEnd;
  this.value = this.value.toUpperCase();
  this.setSelectionRange(start, end);" center :required="true" autocomplete="off" type="text" label="Name"
        placeholder="Please enter the Name" :rules="[{ required: true }]" />
      <van-field v-model="formData.agent_nric" oninput="
  var start = this.selectionStart;
  var end = this.selectionEnd;
  this.value = this.value.toUpperCase();
  this.setSelectionRange(start, end);" name="agent_nric" autocomplete="off" center :required="true" type="text"
        label="NRIC/Passport No" placeholder="Please enter the NRIC/Passport No" :rules="[
          { required: true },
        ]" />
      <van-field v-model="formData.agent_date" name="agent_date" autocomplete="off" center :required="true"
        label="Date: DD-MM-YYYY" placeholder="Please enter DD-MM-YYYY"
        :rules="[{ required: true, pattern, message: 'Please enter the DATE' }]" />
      <van-button v-click-outside="outSide" :disabled="selectData" v-if="!isDone" round block type="info"
        native-type="submit" color="#7C655D">
...
import { getOrdersForms, putOrdersForms, document_check_list_formAagt, third_party_declaration_formAagt, getOrderDetailFor3rdParty, getOrderDetail, getOrderDetailSignature } from "@/api/order";
...
export default {
  ...
  data() {
    return {
      ...
      formData: {
        ...
        nric: this.$store.state.passport_noAagt,
  ...
  mounted() {
    ...
    if (this.$route.query.isShare) {
      this.isFilled = this.$route.query.AagtDocumentCheckListForm
    }
    this.getFormData();
        ...
        .then((res) => {
          ...
          getOrdersForms(res.AagtApplicationForm, { type: "AAGT Application Form" })
            .then((res) => {
              ...
              this.formData.nric = res.passport_no
    ...
    if (this.$store.state.reorder == 1) {
      ...
      if (this.$store.state.reorderdocument_check_list_formAagt > 0) {

        getOrdersForms(this.$store.state.reorderdocument_check_list_formAagt, { type: "AAGT Document Checklist Form" })
          .then((res) => {
            console.log(">>.666.>>", res);
            ...
            console.log(".......99999999999............", res.source_of_fund_file.length)
            console.log("........888888888...........", res.beneficiary_photo_file.length)
            //        for (let j = 0; j < res.source_of_fund_file.length; j++) {

            // this.qaz1.push(res.source_of_fund_file[j])
            // }
            // for (let l = 0; l < res.evidence_of_bank_file.length; l++) {

            // this.qaz2.push(res.evidence_of_bank_file[l])
            // }
            ...
            // this.qaz1.push(res.source_of_fund_file)
            // for (let j = 0; j < res.source_of_fund_file.length; j++) {
            ...
            // this.source_of_fund_addon1.push({url: res.source_of_fund_file[0]})
            // }
            // if(res.source_of_fund_file.length ===2){
            // console.log("...2222....2222...",res.source_of_fund_file[1])
            // this.source_of_fund_addon1.push({url: res.source_of_fund_file[0]})
            // this.source_of_fund_addon2.push({url: res.source_of_fund_file[1]})
            // }
            // if(res.source_of_fund_file.length ===3){
            // console.log("...3333....3333...",res.source_of_fund_file[2])
            // this.source_of_fund_addon1.push({url: res.source_of_fund_file[0]})
            // this.source_of_fund_addon2.push({url: res.source_of_fund_file[1]})
            // this.source_of_fund_addon3.push({url: res.source_of_fund_file[2]})
            // }
            ...
            // this.source_of_fund_addon2.push({url: res.source_of_fund_file[1]})
            // this.source_of_fund_addon3.push({url: res.source_of_fund_file[2]})
            // this.source_of_fund_addon4.push({url: res.source_of_fund_file[3]})
            // }
            // if(res.source_of_fund_file.length > 4){
            // console.log("...44444....44444...",res.source_of_fund_file[3])
            // this.source_of_fund_addon1.push({url: res.source_of_fund_file[0]})
            ...
            // this.source_of_fund_addon3.push({url: res.source_of_fund_file[2]})
            // this.source_of_fund_addon4.push({url: res.source_of_fund_file[3]})
            // this.source_of_fund_addon5.push({url: res.source_of_fund_file[4]})
            // }


            // if(res.evidence_of_bank_file.length ===1){
            // console.log("...1111....1111...",res.evidence_of_bank_file[0])
            // this.source_of_fund_addon1y.push({url: res.evidence_of_bank_file[0]})
            // }
            // if(res.evidence_of_bank_file.length ===2){
            // console.log("...2222....2222...",res.evidence_of_bank_file[1])
            // this.source_of_fund_addon1y.push({url: res.evidence_of_bank_file[0]})
            // this.source_of_fund_addon2y.push({url: res.evidence_of_bank_file[1]})
            // }
            // if(res.evidence_of_bank_file.length ===3){
            // console.log("...3333....3333...",res.evidence_of_bank_file[2])
            // this.source_of_fund_addon1y.push({url: res.evidence_of_bank_file[0]})
            // this.source_of_fund_addon2y.push({url: res.evidence_of_bank_file[1]})
            // this.source_of_fund_addon3y.push({url: res.evidence_of_bank_file[2]})
            // }
            // if(res.evidence_of_bank_file.length ===4){
            // console.log("...44444....44444...",res.evidence_of_bank_file[3])
            // this.source_of_fund_addon1y.push({url: res.evidence_of_bank_file[0]})
            // this.source_of_fund_addon2y.push({url: res.evidence_of_bank_file[1]})
            // this.source_of_fund_addon3y.push({url: res.evidence_of_bank_file[2]})
            // this.source_of_fund_addon4y.push({url: res.evidence_of_bank_file[3]})
            // }
            // if(res.evidence_of_bank_file.length > 4){
            // console.log("...44444....44444...",res.evidence_of_bank_file[3])
            // this.source_of_fund_addon1y.push({url: res.evidence_of_bank_file[0]})
            ...
            // this.source_of_fund_addon3y.push({url: res.evidence_of_bank_file[2]})
            // this.source_of_fund_addon4y.push({url: res.evidence_of_bank_file[3]})
            // this.source_of_fund_addon5y.push({url: res.evidence_of_bank_file[4]})
            // }
  ...
  methods: {
    ...
    getOrderDetailFor3rdParty() {
      // alert("getOrderDetailFor3rdParty")
      let idFor3 = null
      if (this.$store.state.isOverseaSignature) {
      ...
      console.log("__________idFor3_________", idFor3);
      getOrderDetailFor3rdParty({
        order_id: idFor3
      })
        .then(res => {
          console.log("__________ res.AagtThirdPartyDeclarationForm________res_________", res)
          console.log("__________ res.AagtThirdPartyDeclarationForm_________________", res.third_party_declaration_form)
          // if(res.third_party_declaration_form != undefined){
          //   alert("x")
          this.third_party_declaration_Id = res.third_party_declaration_form
          // }
          // else{
          //   alert("y")
          //   this.third_party_declaration_Id = 0
          // }
          console.log("__________ this.third_party_declaration_Id_____________", this.third_party_declaration_Id)
          //this.faqData = res
        })
      //   alert("z")
    ...
    beforeRead81w() {
      ...
      this.isDelete = true
      //alert("1111111111111111111")
      this.qaz1w.splice(0, 1);
      //delete this.qaz[0];
      console.log(".....444444........", this.qaz1w)
      console.log(".....555555........", this.qaz1w.length)
      this.submit()
    },
    ...
    getFormData3(id3) {
      console.log("_______getFormData3__id3_______", id3)
      if (id3 > 0) {
        //    alert("________id3 > 0 ______")
        getOrdersForms(id3, { type: "Third Party Declaration" })
          .then((res) => {
            console.log("________getFormData3___________", res);
            this.formData3 = res;
            let data = JSON.parse(JSON.stringify(this.formData3));
            console.log("________getFormData3______data_____", data);
            putOrdersForms(id3, {
              type: "Third Party Declaration",
              data: JSON.stringify(data),
            }).then((res) => {
              console.log("________getFormData3___putOrdersForms___res_____", res);
    ...
    getFormData() {
      if (this.isFilled > 0) {
        ...
        getOrdersForms(this.isFilled, { type: "AAGT Document Checklist Form" })
          .then((res) => {
            console.log(">>.666.>>", res);
            this.formData = res
            ...
            console.log(".......99999999999............", res.source_of_fund_file.length)
            console.log("........888888888...........", res.beneficiary_photo_file.length)
            for (let a = 0; a < res.source_of_wealth_file.length; a++) {

              this.qaz1w.push(res.source_of_wealth_file[a])
            }
            for (let j = 0; j < res.source_of_fund_file.length; j++) {

              this.qaz1.push(res.source_of_fund_file[j])
            }
            for (let m = 0; m < res.evidence_of_bank_file.length; m++) {

              this.qaz2.push(res.evidence_of_bank_file[m])
            }
            ...
            if (res.source_of_wealth_file.length === 1) {
              console.log("...1111....1111...", res.source_of_wealth_file[0])
              this.source_of_fund_addon1w.push({ url: res.source_of_wealth_file[0] })
            }
            if (res.source_of_wealth_file.length === 2) {
              console.log("...2222....2222...", res.source_of_wealth_file[1])
              this.source_of_fund_addon1w.push({ url: res.source_of_wealth_file[0] })
              this.source_of_fund_addon2w.push({ url: res.source_of_wealth_file[1] })
            }
            if (res.source_of_wealth_file.length === 3) {
              console.log("...3333....3333...", res.source_of_wealth_file[2])
              this.source_of_fund_addon1w.push({ url: res.source_of_wealth_file[0] })
              this.source_of_fund_addon2w.push({ url: res.source_of_wealth_file[1] })
              this.source_of_fund_addon3w.push({ url: res.source_of_wealth_file[2] })
            }
            if (res.source_of_wealth_file.length === 4) {
              console.log("...44444....44444...", res.source_of_wealth_file[3])
              this.source_of_fund_addon1w.push({ url: res.source_of_wealth_file[0] })
              this.source_of_fund_addon2w.push({ url: res.source_of_wealth_file[1] })
              this.source_of_fund_addon3w.push({ url: res.source_of_wealth_file[2] })
              this.source_of_fund_addon4w.push({ url: res.source_of_wealth_file[3] })
            }
            if (res.source_of_wealth_file.length > 4) {
              console.log("...44444....44444...", res.source_of_wealth_file[3])
              this.source_of_fund_addon1w.push({ url: res.source_of_wealth_file[0] })
              ...
              this.source_of_fund_addon3w.push({ url: res.source_of_wealth_file[2] })
              this.source_of_fund_addon4w.push({ url: res.source_of_wealth_file[3] })
              this.source_of_fund_addon5w.push({ url: res.source_of_wealth_file[4] })
            }
    ...
    submit(form) {
      ...
      //  alert("1")
        getOrderDetailSignature({
          ...
          .then((res) => {
            console.log("_____________signature list__________", res);
            if (res.aagt_signatures.AagtApplicationForm_settlor_signature == '' || res.aagt_signatures.AagtApplicationForm_settlor_signature == null ||
              res.aagt_signatures.AagtApplicationForm_settlor_signature_1 == '' || res.aagt_signatures.AagtApplicationForm_settlor_signature_1 == null ||
              res.aagt_signatures.complianceQuestionnaireForm_declaration == '' || res.aagt_signatures.complianceQuestionnaireForm_declaration == null ||
              res.aagt_signatures.AagtLetterOfWishesForm_signature == '' || res.aagt_signatures.AagtLetterOfWishesForm_signature == null ||
              res.aagt_signatures.AagtPdpaForm_eng_signature == '' || res.aagt_signatures.AagtPdpaForm_eng_signature == null ||
              res.aagt_signatures.AagtPdpaForm_malay_signature == '' || res.aagt_signatures.AagtPdpaForm_malay_signature == null
            ) {
      ...
      if (this.isFilled == 0 || this.isFilled == undefined) {
        //this.qaz1.push()
        if (typeof this.source_of_wealth_file1w === "string") {
          // alert("1")
          this.qaz1w.push(this.source_of_wealth_file1w)
        }
        if (typeof this.source_of_wealth_file2w === "string") {
          // alert("2")
          this.qaz1w.push(this.source_of_wealth_file2w)
        }
        if (typeof this.source_of_wealth_file3w === "string") {
          // alert("1")
          this.qaz1w.push(this.source_of_wealth_file3w)
        }
        if (typeof this.source_of_wealth_file4w === "string") {
          // alert("1")
          this.qaz1w.push(this.source_of_wealth_file4w)
        }
        if (typeof this.source_of_wealth_file5w === "string") {
          this.qaz1w.push(this.source_of_wealth_file5w)
          // alert("1")
        }
      ...
      var wsx1w = {};
      this.qaz1w.map((val, index1w) => { return wsx1w[index1w + 1] = val })
      console.log(".....xxxxxxxxxxxxxxxx..1..w.", JSON.stringify(wsx1w))
      ...
      data.source_of_fund_file = JSON.stringify(wsx1)
      data.source_of_wealth_file = JSON.stringify(wsx1w)
      data.evidence_of_bank_file = JSON.stringify(wsx2)
      ...
      if (this.isFilled > 0) {
        ...
        putOrdersForms(this.isFilled, {
          type: "AAGT Document Checklist Form",
          data: JSON.stringify(data),
        }).then((res) => {
          ...
          if (this.isDelete == false) {
            ...
            if (!this.$route.query.isShare) {
              console.log("__________this.formData.third_party_declaration_______", this.formData.third_party_declaration)
              if (this.formData.third_party_declaration == true) {
                console.log("__________this.third_party_declaration_Id_______", this.third_party_declaration_Id)
                if (this.third_party_declaration_Id > 0) {
                  //alert("_______1____if______")
                  this.getFormData3(this.third_party_declaration_Id)
                }
                else {
                  //  alert("_______2____if______")
                  let id4 = null
                  if (this.$store.state.isOverseaSignature) {
                    id4 = this.$store.state.CustomerApplicationId
                  } else {
                    id4 = this.$route.query.orderId
                  }
                  console.log("__________id4_______", id4)
                  console.log("__________data3_______", data3)
                  third_party_declaration_formAagt(id4, data3)
                    .then((res) => {
                      console.log("__________third_party_declaration_formAagt___res____", res)
                    })
                    .catch((err) => { });
        ...
        if (id === "") {
          ...
          document_check_list_formAagt(id, data)
            .then((res) => {
              console.log("----5-5-5-5-5--5-5", res);
              this.isFilled = res.AagtDocumentCheckListForm
              ...
              //   if (this.formData.third_party_declaration == true) {
              //     AagtThirdPartyDeclarationForm(id, data3)
              // .then((res) => {
              ...
              if (this.from == "create") {
                ...
                if (!this.$store.state.isOverseaSignature && !this.$route.query.isShare) {
                  ...
                  if (this.formData.third_party_declaration == true) {
                    ...
                    this.$router.push({
                      path:
                        "/ThirdPartyDeclarationAagt?from=create&orderId=" + this.$route.query.orderId, query: { campanyIndividualName: this.$route.query.campanyIndividualName }
                    })
...
if (!this.formData.signature && !this.$route.query.isShare) {
      ...
      //  alert("xxx1")
        ...
        if (id === "") {
          ...
          console.log("--------finale data -----------------1-------", data);
          document_check_list_formAagt(id, data)
            .then((res) => {
              console.log("----5-5-5-5-5--5-5", res);
              this.isFilled = res.AagtDocumentCheckListForm
    ...
    afterRead31w(file) {
      if (file.file.type.split('/').slice(-1)[0] === "jpeg" || file.file.type.split('/').slice(-1)[0] === "jpg" || file.file.type.split('/').slice(-1)[0] === "png" || file.file.type.split('/').slice(-1)[0] === "pptx" || file.file.type.split('/').slice(-1)[0] === "pdf" || file.file.type.split('/').slice(-1)[0] === "vnd.openxmlformats-officedocument.presentationml.presentation") {
        ...
        let data = new FormData()
        data.append('file', file.file)
        console.log("...ggggggg.2..", this.source_of_wealth_file1w)
        console.log("...ggggggg.3..", file.file)

        uploadFile(data).then(res => {

          this.$toast.success('Success')
          this.source_of_wealth_file1w = res.file
        })
      } else {
        alert("Accept file type are pdf/pptx/jpeg/jpg/png !")
        this.source_of_wealth_file1w = []
    ...
    afterRead32w(file) {
      if (file.file.type.split('/').slice(-1)[0] === "jpeg" || file.file.type.split('/').slice(-1)[0] === "jpg" || file.file.type.split('/').slice(-1)[0] === "png" || file.file.type.split('/').slice(-1)[0] === "pptx" || file.file.type.split('/').slice(-1)[0] === "pdf" || file.file.type.split('/').slice(-1)[0] === "vnd.openxmlformats-officedocument.presentationml.presentation") {
        ...
        uploadFile(data).then(res => {
          ...
          this.$toast.success('Success')
          this.source_of_wealth_file2w = res.file
        })
      } else {
        alert("Accept file type are pdf/pptx/jpeg/jpg/png !")
        this.source_of_wealth_file2w = []
    ...
    afterRead33w(file) {
      if (file.file.type.split('/').slice(-1)[0] === "jpeg" || file.file.type.split('/').slice(-1)[0] === "jpg" || file.file.type.split('/').slice(-1)[0] === "png" || file.file.type.split('/').slice(-1)[0] === "pptx" || file.file.type.split('/').slice(-1)[0] === "pdf" || file.file.type.split('/').slice(-1)[0] === "vnd.openxmlformats-officedocument.presentationml.presentation") {
        ...
        uploadFile(data).then(res => {
          ...
          this.$toast.success('Success')
          this.source_of_wealth_file3w = res.file
        })
      } else {
        alert("Accept file type are pdf/pptx/jpeg/jpg/png !")
        this.source_of_wealth_file3w = []
    ...
    afterRead34w(file) {
      if (file.file.type.split('/').slice(-1)[0] === "jpeg" || file.file.type.split('/').slice(-1)[0] === "jpg" || file.file.type.split('/').slice(-1)[0] === "png" || file.file.type.split('/').slice(-1)[0] === "pptx" || file.file.type.split('/').slice(-1)[0] === "pdf" || file.file.type.split('/').slice(-1)[0] === "vnd.openxmlformats-officedocument.presentationml.presentation") {
        ...
        uploadFile(data).then(res => {
          ...
          this.$toast.success('Success')
          this.source_of_wealth_file4w = res.file
        })
      } else {
        alert("Accept file type are pdf/pptx/jpeg/jpg/png !")
        this.source_of_wealth_file4w = []
    ...
    afterRead35w(file) {
      if (file.file.type.split('/').slice(-1)[0] === "jpeg" || file.file.type.split('/').slice(-1)[0] === "jpg" || file.file.type.split('/').slice(-1)[0] === "png" || file.file.type.split('/').slice(-1)[0] === "pptx" || file.file.type.split('/').slice(-1)[0] === "pdf" || file.file.type.split('/').slice(-1)[0] === "vnd.openxmlformats-officedocument.presentationml.presentation") {
        ...
        uploadFile(data).then(res => {
          ...
          this.$toast.success('Success')
          this.source_of_wealth_file5w = res.file
        })
      } else {
        alert("Accept file type are pdf/pptx/jpeg/jpg/png !")
        this.source_of_wealth_file5w = []
    ...
    afterRead81w(file) {
      ...
      if (file.file.type.split('/').slice(-1)[0] === "jpeg" || file.file.type.split('/').slice(-1)[0] === "jpg" || file.file.type.split('/').slice(-1)[0] === "png" || file.file.type.split('/').slice(-1)[0] === "pptx" || file.file.type.split('/').slice(-1)[0] === "pdf" || file.file.type.split('/').slice(-1)[0] === "vnd.openxmlformats-officedocument.presentationml.presentation") {
        ...
        data.append('file', file.file)
        uploadFile(data).then(res => {
          this.qaz1w.push(res.file)
          this.$toast.success('Success')
          this.source_of_fund_addon1w = res.file
        })
      } else {
        alert("Accept file type are pdf/pptx/jpeg/jpg/png !")
        this.source_of_fund_addon1w = []
    ...
    afterRead91w(file) {
      if (file.file.type.split('/').slice(-1)[0] === "jpeg" || file.file.type.split('/').slice(-1)[0] === "jpg" || file.file.type.split('/').slice(-1)[0] === "png" || file.file.type.split('/').slice(-1)[0] === "pptx" || file.file.type.split('/').slice(-1)[0] === "pdf" || file.file.type.split('/').slice(-1)[0] === "vnd.openxmlformats-officedocument.presentationml.presentation") {
        ...
        uploadFile(data).then(res => {
          this.qaz1w.push(res.file)
          this.$toast.success('Success')
          this.source_of_fund_addon2w = res.file
        })
      } else {
        alert("Accept file type are pdf/pptx/jpeg/jpg/png !")
        this.source_of_fund_addon2w = []
    ...
    afterRead101w(file) {
      if (file.file.type.split('/').slice(-1)[0] === "jpeg" || file.file.type.split('/').slice(-1)[0] === "jpg" || file.file.type.split('/').slice(-1)[0] === "png" || file.file.type.split('/').slice(-1)[0] === "pptx" || file.file.type.split('/').slice(-1)[0] === "pdf" || file.file.type.split('/').slice(-1)[0] === "vnd.openxmlformats-officedocument.presentationml.presentation") {
        ...
        uploadFile(data).then(res => {
          this.qaz1w.push(res.file)
          this.$toast.success('Success')
          this.source_of_fund_addon3w = res.file
        })
      } else {
        alert("Accept file type are pdf/pptx/jpeg/jpg/png !")
        this.source_of_fund_addon3w = []
    ...
    afterRead111w(file) {
      if (file.file.type.split('/').slice(-1)[0] === "jpeg" || file.file.type.split('/').slice(-1)[0] === "jpg" || file.file.type.split('/').slice(-1)[0] === "png" || file.file.type.split('/').slice(-1)[0] === "pptx" || file.file.type.split('/').slice(-1)[0] === "pdf" || file.file.type.split('/').slice(-1)[0] === "vnd.openxmlformats-officedocument.presentationml.presentation") {
        ...
        uploadFile(data).then(res => {
          this.qaz1w.push(res.file)
          this.$toast.success('Success')
          this.source_of_fund_addon4w = res.file
        })
      } else {
        alert("Accept file type are pdf/pptx/jpeg/jpg/png !")
        this.source_of_fund_addon4w = []
    ...
    afterRead112w(file) {
      if (file.file.type.split('/').slice(-1)[0] === "jpeg" || file.file.type.split('/').slice(-1)[0] === "jpg" || file.file.type.split('/').slice(-1)[0] === "png" || file.file.type.split('/').slice(-1)[0] === "pptx" || file.file.type.split('/').slice(-1)[0] === "pdf" || file.file.type.split('/').slice(-1)[0] === "vnd.openxmlformats-officedocument.presentationml.presentation") {
        ...
        uploadFile(data).then(res => {
          this.qaz1w.push(res.file)
          this.$toast.success('Success')
          this.source_of_fund_addon5w = res.file
        })
      } else {
        alert("Accept file type are pdf/pptx/jpeg/jpg/png !")
        this.source_of_fund_addon5w = []
