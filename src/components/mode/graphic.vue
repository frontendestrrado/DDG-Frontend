<template>
  <div
    class="index-graphic"
    :style="{
      backgroundColor: graphicData.bg_color,
      marginBottom: graphicData.layout == 1 ? '10px' : '0',
    }"
  >
    <div class="index-graphic-div">
      <van-row class="columnboxAbout" :gutter="$store.state.isPC ? 20 : 0">
        <van-col
          :span="showClass"
          v-for="(graphic, index) in graphicData.imageText"
          :key="index"
        >
          <div
            :class="
              'imgTextLayout layoutImg_' +
              graphicData.layout_images_text +
              ' layoutImgOne_' +
              graphicData.layout
            "
            @click="goView(graphic.path, graphic.text)"
          >
            <div
              :class="graphicData.layout == 1 ? 'imgTextBox1' : 'imgTextBox'"
            >
              <van-image
                v-show="graphic.image"
                :style="graphicData.layout == 3 ? 'width:50%' : 'width:90%'"
                class="image"
                :src="graphic.image"
                fit="contain"
              />
              <span
                :class="'bottomText layout_' + graphicData.layout_font_in_image"
                :style="{
                  fontSize: graphicData.font_in_image_size + 'px',
                  color: graphicData.font_in_image_color,
                  textAlign: graphicData.align,
                }"
                >{{ graphic.font_in_image }}</span
              >
            </div>
            <pre
              class="textPre"
              :style="{
                fontSize: graphicData.font_size + 'px',
                color: graphicData.color,
                textAlign: graphicData.align,
                paddingLeft: '10px',
                lineHeight: graphicData.layout == 1 ? '25px' : '25px',
              }"
            ><!--
