          ...
          <van-cell is-link class="textContent" @click="$store.commit('changePage',{tabbar: item.link, title: item.title});$router.push({path: item.link})">
            <div
              :style="{
                ...
                color: textData.title_color,
                fontWeight: textData.title_bold == 1 ? 'bold' : 'normal',
              }"
            >
              {{ item.title }}
            </div>
            <div
              :style="{
                fontSize: textData.content_size + 'px',
                color: textData.content_color,
              }"
            >
              {{ item.content }}
            </div>
